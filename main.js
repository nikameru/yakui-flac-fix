...
// FLAC METADATA EDITOR - credit to AHOHNMYC: https://github.com/AHOHNMYC/FLACMetadataEditor/
...
const FLACMetadataEditor = (() => {
    ...
    const _version = '0.0.2.1-modified';
    ...
    class _FLACMetadataEditor {
        ...
        editComment(field, value) {
            this.removeComment(field);
            this.addComment(field, value);
        ...
        _parseMetadata() {
            ...
            while (!lastBlock && offset < bytes.length) {
                if (iteration++ > 42) throw new RangeError('Too much METADATA_BLOCKS. Looks like file is corrupted');
...
if (dir == "insert your path here") {
    return console.log("Please specify your path first!");
}
...
fs.readdir(dir, (err, albums) => {
    ...
    albums.forEach(album => {
        if (album.includes("YAKUITHEMAID")) {
            fs.renameSync(`${dir}/${album}`, `${dir}/${album.replace("YAKUITHEMAID", "Yakui The Maid")}`);
        }

        const albumPath = `${dir}/${album.replace("YAKUITHEMAID", "Yakui The Maid")}`;

        fs.readdir(albumPath, (err, tracks) => {
            if (err) console.log(err);

            tracks.forEach(track => {
                if (track.includes("YAKUITHEMAID") && !track.includes(".jpg")) {
                    process.stdout.write(`Processing ${track}...`);
                    
                    try {
                        const trackBuffer = fs.readFileSync(`${albumPath}/${track}`);
                        const metadata = new FLACMetadataEditor(trackBuffer);

                        metadata.editComment("ARTIST", "Yakui The Maid");
                        metadata.editComment("ALBUMARTIST", "Yakui The Maid");
                        metadata.serializeMetadata();

                        const resultBuffer = Buffer.from(metadata.arrayBuffer);

                        fs.writeFileSync(`${albumPath}/${track.replace("YAKUITHEMAID", "Yakui The Maid")}`, resultBuffer);
                        fs.unlinkSync(`${albumPath}/${track}`);
